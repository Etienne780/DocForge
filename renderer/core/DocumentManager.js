...
export function serializeProject(project, kind) {
  ...
  // { [tabFolderName]: { [nodeFileName]: { name, content } } } - written as one
  // .md file per node (see ElectronDocumentIOAdapter._writeTabFolders). Node
  // file names are unique per tab (flat namespace - all of a tab's nodes land
  // in the same folder regardless of nesting depth), tab folder names are
  // unique per project.
  const nodeContents = {};
