...
import { getActiveProject, getActiveTab, flattenNodes } from '../../../data/ProjectManager.js';
import { parseMarkdown } from '../../EditorArea/helpers/MarkdownParser.js';
 ...
 * @param {string} value
 */
export function applyCSSVariable(variableName, value) {
  const property = variableName.startsWith('--') ? variableName : `--${variableName}`;
  document.documentElement.style.setProperty(property, value);
  const theme = { ...state.get('theme'), [property]: value };
  state.set('theme', theme);
}

/**
 * Applies all stored theme overrides from state to the document root.
 * Called on page load to restore user customizations.
 */
export function applyStoredTheme() {
  const theme = state.get('theme');
  for (const [property, value] of Object.entries(theme)) {
    if (property === 'preview-font-size') {
      const previews = document.querySelectorAll('[data-preview-panel]');
      previews.forEach(el => { el.style.fontSize = `${value}px`; });
    } else {
      document.documentElement.style.setProperty(property, value);
    }
  }
...
export function applyPreviewFontSize(sizeInPixels) {
  ...
  const theme = { ...state.get('theme'), 'preview-font-size': sizeInPixels };
  state.set('theme', theme);
}
...
export function resetTheme() {
  ...
  const previews = document.querySelectorAll('[data-preview-panel]');
  previews.forEach(el => { el.style.fontSize = ''; });
  state.set('theme', {});
}
...
/**
 * Returns the inner SVG markup for the dark/light mode toggle button.
 * @param {boolean} isDark
 * @returns {string}
 */
export function getModeIconSVG(isDark) {
  if (isDark) {
